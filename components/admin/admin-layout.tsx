...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const NavigationLinks = () => (
    <nav className="p-4 space-y-2">
      ...
      <Link href="/admin/teachers" onClick={() => setMobileMenuOpen(false)}>
        <Button variant="ghost" className="w-full justify-start text-white hover:bg-white/10">
          إدارة المدرسين
        </Button>
      </Link>
      <Link href="/admin/classes" onClick={() => setMobileMenuOpen(false)}>
        <Button variant="ghost" className="w-full justify-start text-white hover:bg-white/10">
          إدارة الفصول
        </Button>
      </Link>
      <Link href="/admin/students" onClick={() => setMobileMenuOpen(false)}>
        <Button variant="ghost" className="w-full justify-start text-white hover:bg-white/10">
          إدارة الطلاب
        </Button>
      </Link>
      <Link href="/admin/grade-schema" onClick={() => setMobileMenuOpen(false)}>
        <Button variant="ghost" className="w-full justify-start text-white hover:bg-white/10">
          لائحة الدرجات
        </Button>
      </Link>
    ...
    <div className="flex h-screen bg-[#2d2520]" dir="rtl">
      <aside className="hidden lg:block w-64 bg-black/60 backdrop-blur-sm text-white shadow-lg">
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-sm text-white">لوحة التحكم الإدارية</p>
        </div>
        <NavigationLinks />
      </aside>
            ...
            <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
              <SheetTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="lg:hidden hover:bg-white/10 text-white transition-colors"
                >
              ...
              <SheetContent
                side="right"
                className="w-64 bg-black/90 backdrop-blur-md text-white border-white/10 p-0 z-50"
              >
                <div className="p-6 border-b border-white/10">
                  <h1 className="text-2xl font-bold text-[#d4a574]">منصة الدرجات</h1>
                  <p className="text-sm text-white">لوحة التحكم الإدارية</p>
